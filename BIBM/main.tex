...
\begin{figure}
	...
	\caption{Flowchart of data loader, barcode extraction and quality control in scSpark.} \label{fig1}
\end{figure}
\fi
...
% RDD可以是(key, value)（像数据结构中的map）这样的格式，为了下一步的处理，将FASTQ R1 RDD处理为(R1 cell barcode, R1 read name + R1 UMI + R1第四行 + R2 xxx )这样的格式
...
In this step, scSpark splits cell barcode and UMI when scSpark load FASTQ file to FASTQ RDD.
