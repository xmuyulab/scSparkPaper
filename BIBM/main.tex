...
Due to the increase in data volume of a scRNA-seq study, the speed of scRNA-seq data processing pipeline has become a major bottleneck to large-scale scRNA-seq study. 
...
By leveraging Apache Spark's in-memory computing capability, scSpark significantly improves the processing speed of scRNA-seq data, and achieves 5\-20 times faster than state-of-the-art processing pipelines under the same CPU core consumption.
...
Biological evaluation also confirmed that the results produced by scSpark are highly consistent with existing scRNAseq pipelines.
\end{abstract}
...
To enable efficient bioinformatics processing of sequencing data produced for scRNA-seq studies, various scRNA-seq data processing pipelines have been developed.
...
SparkBWA~\cite{Abun2016SparkBWA} exploits the capabilities of Spark to boost the performance of one of the most widely adopted aligners for NGS data, the Burrows-Wheeler Aligner (BWA). 
GPF~\cite{Li2018Highperformance} is a fast in-memory computing framework designed based on Spark for implementing NGS data processing pipelines in a cloud-computing environment. 
Following the success of cloud-based implementation of NGS data processing pipelines, Falco~\cite{Yang2017Falco} concatenates the genome aligner and transcript quantification software tools using Spark for scRNA-seq data preprocessing.
...
More specifically, by implementing and integrating multiple procedures of a standard scRNA-seq data processing pipeline with the aid of in-memory computing characteristics of Spark, scSpark eliminates the need for laborious disk read/write operations associated with traditional bioinformatics tools.
As a result, scSpark is able to improve the scRNA-seq data processing speed by more than 10 folds compared with other state-of-the-art software tools under the same CPU core consumption. In addition, by harnessing the merit of the parallel computing capability of Spark engine, scSpark further enables users to distribute their scRNA-seq data processing workloads to multiple computational nodes, thus dramatically increases their processing throughput of scRNA-seq data for large-scale studies. 
...
The overall workflow of scSpark follows closely with that of UMI-tools.
Briefly, the scRNA-seq data processing has three major steps: (i) data loading with whitelist control, (ii) alignment of extracted reads to a reference genome, (iii) transcript quantification.
...
scSpark extracts cell barcode from the first read in a read-pair, which are then used as the key to index the UMI and transcript sequence from the read-pair.
...
However, conventional STAR alignment procedure needs to read raw reads from FASTQ files and write alignment results to Sequence Aligment/Map (SAM)~\cite{li2009sequence} files, which brings extensive disk access consuming a large amount of time.
Here we use the Java Native Interface (JNI)~(\url{https://docs.oracle.com/javase/8/docs/technotes/guides/jni/}) to directly feed FASTQ RDD to STAR, and further transfer the results to the SAM RDD after alignment (Fig.~\ref{fig2}).
Note that all these steps are performed throughly in an in-memory computing manner without any disk access.
In addition, to balance the workloads on each node during parallel processing, we use the \textit{repartition} function before alignment to split FASTQ RDDs into \textit{N} subsets where \textit{N} denotes the number of nodes available. 
...
First, we used the \textit{textFile} function to load gene transfer format (GTF) file in memory as GTF RDDs (Fig.~\ref{fig3}). 
Then all the SAM RDDs and GTF RDDs were grouped by chromosome into Grouped SAM RDDs and Grouped GTF RDDs respectively for parallel processing, and the gene name could be assigned to each aligned read using the \textit{join} function.
Finally, scSpark reimplemented the directional algorithm designed by UMI-tools for transcript quantification using \textit{flatMap}, followed by writing the final expression matrix to the disk for further downstream analysis.
...
In all experiments, we processed three tradition pipelines and scSpark with same expect cell number and barcode pattern to ensure each pipeline handle same data volume.
And we ensure each pipeline have sufficient in-memory to ensure process time doesn's influence by in-memory bottleneck.
