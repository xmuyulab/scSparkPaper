...
Dataset pbmc\_human, 10X Genomics v3 10k peripheral blood mononuclear cell (PBMC) from a healthy donor, which contain 640 millions records.
Dataset pbmc\_rat, 10X Genomics 10k Rat PBMCs Multiplexed from a  Wistar rat strain~(\url{www.10xgenomics.com/resources/datasets/10-k-rat-pbm-cs-multiplexed-2-cm-os-3-1-standard-6-0-0}), which contain 289.3 millions records.
Dataset pbmc\_monkey, 10X Genomics 10k Monkey PBMCs Multiplexed from a Rhesus monkey strain~(\url{www.10xgenomics.com/resources/datasets/10-k-monkey-pbm-cs-multiplexed-2-cm-os-3-1-standard-6-0-0}), which contain 261.5 millions records.
...
\begin{table}
	...
	\begin{tabular}{l | l | l | l | l | l}
		\hline
		Pipeline & Dataset & Cores & Barcode and UMI correct, Quality control & Align & Count \\
		\hline
		UMI-tools & pbmc\_human & 64 cores & 6550 & 492 & 242 \\
		scSpark & pbmc\_human & 4*16 cores & 130 & 132 & 93 \\
		UMI-tools & pbmc\_rat & 64 cores & 6537 & 504 & 218 \\
		scSpark & pbmc\_rat & 4*16 cores & 137 & 117 & 72 \\
		UMI-tools & pbmc\_monkey & 64 cores & 7050 & 616 & 143 \\
		scSpark & pbmc\_monkey & 4*16 cores & 129 & 227 & 35 \\
		\hline
	\end{tabular} }
\end{table}
...
Contrast with tradition pipelines, scSpark not only can perform well in same CPU cores but also scSpark can get improve when CPU cores number increase.
% CPU数增加对传统流程性能影响小

As shown in Fig 6, we found three tradition pipelines can get improve when CPU cores number increase.
But this speed improve will converge quickly.
We used tradition pipelines' speedup rate divided CPU cores number increase rate as baseline to examify scSpark's scalability.
...
Furthermore, we also found STAR's mapping speed can get improve along with CPU's cores increase, but would converge after touching the threshold.
...
We also compared scSpark's result with UMI-tools, and found scSpark's result
