...
First, we load each scRNA-seq read in FASTQ format~\cite{cock2010sanger} into one RDD using Hadoop-BAM~\cite{hadoopBAM}.
...
Similar to CellRanger, UMI-tools and STARsolo, we use STAR as the aligner in scSpark. However, conventional STAR alignment procedure needs to read raw reads from FASTQ files and write alignment results to Sequence Aligment/Map (SAM)~\cite{li2009sequence} files, which brings extensive disk access consuming a large amount of time.
Here we use the Java Native Interface (JNI)~(\url{https://docs.oracle.com/javase/8/docs/technotes/guides/jni/}) to directly feed FASTQ RDD to STAR, and further save the results to the SAM RDD after alignment (Fig.~\ref{fig2}). Note that all these steps are performed thoroughly in an in-memory computing manner without any disk access. In addition, to balance the workloads on each node during parallel processing, we use the \textit{repartition} function before alignment to split FASTQ RDDs into \textit{N} subsets where \textit{N} denotes the number of nodes available. 
